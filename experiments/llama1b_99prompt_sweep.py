#!/usr/bin/env python3
"""
99-prompt alpha sweep — Llama-3.2-1B, extended alphas up to 10.
"""
...
ALPHAS = [-1.0, -0.5, 0.0, 0.5, 1.0, 2.0, 3.0, 5.0, 7.0, 10.0]
...
def main():
    ...
    for alpha in ALPHAS:
        ...
        for i, prompt in enumerate(PROMPTS):
            ...
            if (i + 1) % 25 == 0:
                print(f"  alpha={alpha}: {i+1}/{len(PROMPTS)} done, {refusals} refusals")

        pct = refusals / len(PROMPTS) * 100
        bar = '█' * int(pct / 2.5)
        print(f"  alpha={alpha:>5.1f}: {refusals:>3}/{len(PROMPTS)} = {pct:>5.1f}%  {bar}")

        all_results[str(alpha)] = {
            "n_refusals": refusals, "n_total": len(PROMPTS),
            "pct": pct, "samples": samples,
        }

    output = OUTPUT_DIR / "llama1b_99prompt_sweep.json"
    with open(output, "w") as f:
        json.dump({"model": MODEL, "n_neurons": n_neurons, "results": all_results}, f, indent=2)
    print(f"\nSaved to {output}")
    ...
    for alpha in ALPHAS:
        ...
        bar = '█' * int(r['pct'] / 2.5)
        print(f"  alpha={alpha:>5.1f}: {r['n_refusals']:>3}/{r['n_total']} = {r['pct']:>5.1f}%  {bar}")

    best = min(all_results.keys(), key=lambda k: all_results[k]['pct'])
    print(f"\nBest alpha={best} ({all_results[best]['pct']:.1f}%) — samples:")
    for s in all_results[best]['samples'][:5]:
