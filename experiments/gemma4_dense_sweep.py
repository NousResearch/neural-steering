#!/usr/bin/env python3
"""
99-prompt dense sweep — Gemma-4-E2B, fine-grained alphas [-1, 1].
"""
...
def main():
    ...
    for alpha in ALPHAS:
        ...
        bar = '█' * int(pct / 2.5)
        print(f"  alpha={alpha:>5.2f}: {refusals:>3}/{len(PROMPTS)} = {pct:>5.1f}%  {bar}")
    ...
    output = OUTPUT_DIR / "gemma4_dense_sweep.json"
    with open(output, "w") as f:
        json.dump({"model": MODEL, "n_neurons": n_neurons, "results": all_results}, f, indent=2)
    print(f"\nSaved to {output}")

    print(f"\nGemma-4-E2B DENSE SWEEP RESULTS:")
    for alpha in ALPHAS:
        r = all_results[str(alpha)]
        bar = '█' * int(r['pct'] / 2.5)
        print(f"  alpha={alpha:>5.2f}: {r['n_refusals']:>3}/{r['n_total']} = {r['pct']:>5.1f}%  {bar}")
