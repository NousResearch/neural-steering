#!/usr/bin/env python3
"""
Extended alpha sweep for Qwen2.5-3B and Gemma-4-E2B.
Tests high alphas to see refusal suppression curve.
"""
...
ALPHAS = [-1.0, -0.5, 0.0, 0.5, 1.0, 2.0, 3.0, 5.0, 7.0]
...
def run_model(model_name, model_id, max_memory=None):
    print(f"\n{'='*60}")
    print(f"  {model_name}")
    print(f"{'='*60}")
    ...
    steerer = NeuronSteerer(model_id, **kwargs)
    ...
    print(f"  Found {len(circuit.neurons)} refusal neurons")
    ...
    for alpha in ALPHAS:
        ...
        for i, prompt in enumerate(PROMPTS):
            ...
            if (i + 1) % 25 == 0:
                print(f"    alpha={alpha}: {i+1}/99 done, {refusals} refusals")

        pct = refusals / len(PROMPTS) * 100
        all_results[str(alpha)] = {"n_refusals": refusals, "n_total": len(PROMPTS), "pct": pct, "samples": samples}
        bar = '█' * int(pct / 2.5)
        print(f"    alpha={alpha:>5.1f}: {refusals:>3}/99 = {pct:>5.1f}%  {bar}")

    return {"model": model_name, "n_neurons": len(circuit.neurons), "results": all_results}


def main():
    models = [
        ("Qwen2.5-3B", "Qwen/Qwen2.5-3B-Instruct", None),
        ("Gemma-4-E2B", "google/gemma-4-E2B", {0: "9GiB", 1: "9GiB", "cpu": "30GiB"}),
    ]

    all_results = {}
    for name, model_id, max_mem in models:
        try:
            result = run_model(name, model_id, max_mem)
            all_results[name] = result
            out = OUTPUT_DIR / f"{name.lower().replace('-','_')}_extended_alpha.json"
    ...
    # Combined summary
    print(f"\n{'='*60}")
    print("  EXTENDED ALPHA SWEEP SUMMARY")
    print(f"{'='*60}")
    header = f"  {'Alpha':>6}"
    for name in all_results:
        header += f"  {name:>12}"
    print(header)
    print(f"  {'-'*6}" + f"  {'-'*12}" * len(all_results))
    for alpha in ALPHAS:
        row = f"  {alpha:>6.1f}"
        for name in all_results:
            r = all_results[name]['results'].get(str(alpha), {})
            row += f"  {r.get('pct', 0):>10.1f}%"
        print(row)
