#!/usr/bin/env python3
"""
Binary Yes/No Ablation Studies for Qwen3-4B
==============================================
Framing behavioral tests as binary Yes/No questions gives cleaner
causal signal than arbitrary first-token measurement.

Each behavior tested with:
1. Binary prompt framed as yes/no question
2. Measure P("Yes") and P("No") under normal, ablated, amplified
3. Dose-response sweep
4. Multi-prompt generalization

Usage:
    cd neural-steering
    source neuralvenv/bin/activate
    python experiments/run_binary_ablations_qwen3.py
"""
...
def measure(steerer, prompt, circuit=None, multiplier=1.0):
    return steerer.next_token_probs(prompt, BINARY_TOKENS, circuit=circuit, multiplier=multiplier)
...
def run_behavior(steerer, name, circuit_prompts, test_prompts, expected_direction):
    """Run a full binary ablation study for one behavior.

    expected_direction: 'yes_up' means we expect P(Yes) to increase under ablation
    """
    print(f"\n{'=' * 60}")
    print(f"{name.upper()}")
    print(f"{'=' * 60}")
    ...
    for prompt in test_prompts:
        ...
        print(f"\n  '{prompt}'")
        print(f"    Yes: {normal[' Yes']:.4f} -> ablated={ablated[' Yes']:.4f} ({yes_delta:+.4f})")
        print(f"    No:  {normal[' No']:.4f} -> ablated={ablated[' No']:.4f} ({no_delta:+.4f})")
    ...
    # Dose-response on first test prompt
    ...
    for alpha, probs in sorted(dr.items(), key=lambda x: float(x[0])):
        print(f"    alpha={float(alpha):.2f}: Yes={probs[' Yes']:.4f}  No={probs[' No']:.4f}")
...
def run_random_control(steerer):
    """Random neuron control for binary prompts."""
    print(f"\n{'=' * 60}")
    ...
    for prompt in test_prompts:
        ...
        max_delta = max(abs(rand[t] - normal[t]) for t in BINARY_TOKENS)
        results[prompt] = max_delta
        print(f"  '{prompt[:50]}...' max_delta={max_delta:.4f}")
...
def main():
    ...
    # Save
    output_file = OUTPUT_DIR / "binary_ablation_Qwen2_5_3B.json"
