#!/usr/bin/env python3
"""
Large-scale verification of negative alpha steering.
99 prompts × 4 alphas (-1, -0.5, 0, 1)
"""
...
def main():
    ...
    for alpha in ALPHAS:
        print(f"\n{'='*60}")
        print(f"α={alpha} — {len(PROMPTS)} prompts")
        print(f"{'='*60}")
        ...
        for i, prompt in enumerate(PROMPTS):
            ...
            if is_ref:
                refusals += 1
            samples.append({"prompt": prompt, "gen": gen[:150], "refusal": is_ref})

            if (i + 1) % 20 == 0:
                print(f"  {i+1}/{len(PROMPTS)} done, {refusals} refusals")
        ...
        bar = '█' * int(pct / 2.5)
        print(f"\n  TOTAL: {refusals}/{len(PROMPTS)} = {pct:.1f}%  {bar}")

    # Save
    output_file = OUTPUT_DIR / "negative_alpha_100.json"
    ...
    print("FINAL RESULTS")
    print(f"{'='*60}")
    for alpha in sorted(all_results.keys(), key=float):
        r = all_results[alpha]
        bar = '█' * int(r['pct'] / 2.5)
        print(f"  α={float(alpha):>6.1f}: {r['n_refusals']:>3}/{r['n_total']} = {r['pct']:>5.1f}%  {bar}")

    # Show sample outputs at the best alpha
    best = min(all_results.keys(), key=lambda k: all_results[k]['pct'])
    print(f"\n=== Best α={best} — sample outputs ===")
    for s in all_results[best]['samples'][:10]:
